...
class Securities:
    ...
    def add(self, name, aliases, isin_id, yahoo_id, type, interactive=False):
        ...
        if not already_exists:
            ...
            if not interactive_success:
                ...
                self.data.c.execute(
                    'INSERT INTO stocks(id, name, aliases, isin_id, yahoo_id, type) VALUES (?, ?, ?, ?, ?, ?)',
                    (uuid.uuid4(), name, '::'.join(aliases), isin_id, yahoo_id, type))
        else:
    ...
    def change_stock(self, isin_id, sec):
        ...
        for num, item in enumerate(self.securities):
            if isin_id.lower() in item.isin_id.lower():
                ...
                self.data.c.execute(
                    'UPDATE stocks SET name = ?, aliases = ?, isin_id = ?, yahoo_id = ?, type = ? WHERE isin_id = ?',
                    (sec.name, '::'.join(sec.aliases), sec.isin_id, sec.yahoo_id, sec.type, isin_id))
                self.data.commit()
                break
        return found
    ...
    def merge_stocks_from_isin(self, main_isin, secondary_isin):
        ...
        if yes_no == 'yes':
            self.data.c.execute('''UPDATE transactions SET stock_id = ? WHERE stock_id = ?''', (
            self.get_stock_id_from_isin_id(main_isin), self.get_stock_id_from_isin_id(secondary_isin)))

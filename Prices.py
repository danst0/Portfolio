...
class Prices:
	...
	def delete_prices(self, de_id):
		stock_id = self.secs.get_stock_id_from_de_id(de_id)
		self.data.c.execute('''DELETE FROM prices WHERE stock_id = ?''', (stock_id, ))
	def get_dates_and_prices(self, de_id, from_date, to_date=datetime.date.today().strftime('%Y-%m-%d')):
		stock_id = self.get_stock_id_from_de_id(de_id)
		if from_date == None:
		...
		return dates, values
	def find_split_date(self, de_id):
		prices = self.get_prices(de_id)
		old_price = None
	...
	def update(self, de_id, date, price):
		id = self.secs.get_stock_id_from_de_id(de_id)
		if id not in self.numbers.keys():
			self.numbers[id] = {}
		self.numbers[id][date] = price
	...
	def get_price(self, id, date):
		...
		for i in range(4):
			...
			try:
				...
				pass
		return price
	def get_prices(self, de_id):
		id = self.secs.get_stock_id_from_de_id(de_id)
		prices = None
		try:
			prices = self.numbers[de_id]
		except:
			pass
		return prices
